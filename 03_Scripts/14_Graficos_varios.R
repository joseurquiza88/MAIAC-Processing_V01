...
ggsave("D:/Josefina/paper_git/paper_maiac/plot/V04/MAIAC-C61-Latam-R2_v02.png",a,
       ...
       dpi = 500)
###
plot_3<- # Crea el grafico de dispersion con ggplot2
...
# rmse<- ggplot(metricas_subset, aes(x = estacion, y = valor, color = espacial))+
rmse<- ggplot(metricas_subset, aes(x =espacial , y = valor, color = estacion ))+
  geom_point(alpha=0.6, fill="black",size=3) +
  scale_color_manual(values = c("#005a32", "#fd8d3c","#99000d","#023858","#ce1256","#3f007d")) +
...
rmse
ggsave("D:/Josefina/paper_git/paper_maiac/plot/V04/MAIAC-C61-Latam-RMSE_V03.png",rmse,
       width = 10,
       height = 8,
       units = "cm",
       dpi = 500)
...
# Ajustar el código para asignar los colores manualmente y reducir el ancho de los boxplots
rmse<- ggplot(metricas_subset, aes(x = estacion, y = valor, color = estacion))+
  ...
  scale_y_continuous(limits=c(0, 0.14),breaks = c(0,0.02,0.04,0.06,0.08,0.1,0.12,0.14))+#,0.16,0.18,0.2,
...
ggsave("D:/Josefina/paper_git/paper_maiac/plot/V04/MAIAC-C61-Latam-RMSE_V02.png",rmse,
       width = 10,
       height = 8,
       units = "cm",
       dpi = 500)
...
#theme_minimal()
plot <- # Crea el gr?fico de dispersi?n con ggplot2
...
plot_tot2 <- grid.arrange(e,f,ncol=2)#,heights=c(2,0.7))
#plot_tot3 <- grid.arrange(a,plot_tot2,ncol=1) 
ggsave("D:/Josefina/paper_git/paper_maiac/plot/V03/MAIAC-C61-Latam-RMSE-TemporalWindow.png",rmse,
       width = 10,
       height = 8,
       units = "cm",
       dpi = 500)
...
bias<- ggplot(metricas_subset, aes(x = espacial, y = valor,  color = estacion )) +
# bias<- ggplot(metricas_subset, aes(x = estacion, y = valor,  color = espacial)) +
  geom_point(alpha=0.6, fill="black",size=3) +
...
ggsave("D:/Josefina/paper_git/paper_maiac/plot/V04/MAIAC-C61-Latam-Bias_V03.png",bias,
       width = 10,
       height = 8,
       units = "cm",
       dpi = 500)
...
# Ajustar el código para asignar los colores manualmente y reducir el ancho de los boxplots
bias<- ggplot(metricas_subset, aes(x = estacion, y = valor, color = estacion))+
  ...
  # Personalización de los ejes
  scale_y_continuous(limits=c(-0.07, 0.1))+#,breaks = c(-0.7,-0.06,-0.04,-0.02,0,0.02,0.04,0.06,0.08,0.1,0.12,0.14,0.16,0.18,0.2,0.22,0.24,0.26,0.28))+
...
ggsave("D:/Josefina/paper_git/paper_maiac/plot/V04/MAIAC-C61-Latam-Bias_V02.png",rmse,
       width = 10,
       height = 8,
       units = "cm",
       dpi = 500)
...
#theme(legend.position = "none")
  ...
  guides(
    color = guide_legend(override.aes = list(size = 5))  # Solo cambia el tamaño de los símbolos en la leyenda de color
  )
...
plot_tot2 <- grid.arrange(e,f,ncol=2)#,heights=c(2,0.7))
#plot_tot3 <- grid.arrange(a,plot_tot2,ncol=1) 
ggsave("D:/Josefina/paper_git/paper_maiac/plot/V03/MAIAC-C61-Latam-Bias-TemporalWindow_Leyenda.png",f,
       width = 10,
       height = 12,
       units = "cm",
       dpi = 500)
...
metricas_subset <- metricas[metricas$metrica == metrica_interes,]
#shape = temporal, 
# reu<- ggplot(metricas_subset, aes(x = estacion, y = valor, color = espacial)) +
reu<- ggplot(metricas_subset, aes(x = espacial, y = valor, color = estacion )) +
  geom_point(alpha=0.6, fill="black",size=3) +
  scale_color_manual(values = c("#005a32", "#fd8d3c","#99000d","#023858","#ce1256","#3f007d")) +
  ...
  labs(#title = "MODIS USA",
       ...
       shape = "Temporal window") +theme_bw()+  theme(legend.title = element_text(#family = "Roboto",
         ...
         size = 14,
         face = 2))+  # Elimina la leyenda
  theme(legend.text = element_text(size =12))+
theme_classic()+theme(legend.position = "none")
reu
ggsave("D:/Josefina/paper_git/paper_maiac/plot/V04/MAIAC-C61-Latam-REU.png",reu,
       width = 10,
       height = 8,
       units = "cm",
       dpi = 500)
...
reu<- ggplot(metricas_subset, aes(x = estacion, y = valor, color = estacion))+
  ...
  # scale_y_continuous(limits=c(0, 300),breaks = c(0,20,40,60,80,100,120,140))+
  scale_y_continuous(limits=c(0, 300),breaks = c(0,40,80,120,160,200,240,280))+
  # Etiquetas
  labs(
    x = "Station",
...
ggsave("D:/Josefina/paper_git/paper_maiac/plot/V04/MAIAC-C61-Latam-REU_v02.png",reu,
       width = 10,
       height = 8,
       units = "cm",
       dpi = 500)
...
e<- # Crea el gr?fico de dispersi?n con ggplot2
  ggplot(metricas_subset, aes(y = valor, x = espacial, size = temporal, color=estacion)) +
  geom_point(shape=1,stroke = 0.2) +
  scale_y_continuous(limits=c(0, 140),breaks = c(0,20,40,60,80,100,120,140))+
...
e
f<- # Crea el gr?fico de dispersi?n con ggplot2
  ggplot(metricas_subset, aes(y = valor, x = temporal,  size =espacial,color=estacion )) +
  geom_point(shape=1,stroke = 0.2) +
  scale_y_continuous(limits=c(0, 140),breaks = c(0,20,40,60,80,100,120,140))+
  labs(title = "MODIS USA",
