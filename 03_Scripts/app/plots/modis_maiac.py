...
def plot_modis_maiac(
    ...
    Estilo idéntico a plot_comparativa_modis_maiac.
    """
    ...
    # Modelos
    # =========================
    model_modis = LinearRegression().fit(
        x_modis.values.reshape(-1, 1), y_modis
    )
    model_maiac = LinearRegression().fit(
        x_maiac.values.reshape(-1, 1), y_maiac
    )
    ...
    ax.plot(
        x_line,
        model_modis.predict(x_line.reshape(-1, 1)),
        color=color_modis,
        lw=2
    )
    ...
    ax.plot(
        x_line,
        model_maiac.predict(x_line.reshape(-1, 1)),
        color=color_maiac,
        lw=2
    )
