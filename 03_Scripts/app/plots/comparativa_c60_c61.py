...
def plot_comparativa_c60_c61(
    ...
    """
    Comparativa MAIAC C6.0 vs C6.1 frente a AERONET para una ciudad.

    Parámetros
    ----------
    file : str o Path
        CSV con datos MAIAC + AERONET
    ciudad : str
        Nombre de la estación / ciudad
    color_c61 : str
        Color MAIAC C6.1
    color_c60 : str
        Color MAIAC C6.0
    ...
    ax.plot(
        x_line,
        model_c60.predict(x_line.reshape(-1, 1)),
        color=color_c60,
        lw=2
    )
    ax.plot(
        x_line,
        model_c61.predict(x_line.reshape(-1, 1)),
        color=color_c61,
        lw=2
    )
    ...
    # ax.set_title(f"{ciudad} – MAIAC C6.0 vs C6.1", fontsize=12)
